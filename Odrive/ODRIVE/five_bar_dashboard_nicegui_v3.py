...
class TrapezoidalProfile:
    """
    1-D trapezoidal velocity profile between `start` and `end`.
    Falls back to a triangular profile automatically if there isn't enough
    distance to reach vmax before having to decelerate again.
    """

    def __init__(self, start, end, vmax, amax):
        self.start = start
        self.end = end
        self.direction = 1.0 if end >= start else -1.0
        ...
        if self.distance <= 0:
            self.t_acc = self.t_flat = self.t_dec = self.duration = 0.0
            self.vpeak = 0.0
            return

        if 2 * d_acc >= self.distance:
            # Not enough room to reach vmax -> triangular profile
            t_acc = math.sqrt(self.distance / amax)
            self.t_acc = t_acc
            self.t_flat = 0.0
            self.vpeak = amax * t_acc
        else:
            self.t_acc = t_acc
            d_flat = self.distance - 2 * d_acc
            self.t_flat = d_flat / vmax
            self.vpeak = vmax

        self.t_dec = self.t_acc
        self.duration = self.t_acc + self.t_flat + self.t_dec
    ...
    def position(self, t):
        ...
        if t < self.t_acc:
            ...
            d = 0.5 * a * self.t_acc ** 2 + self.vpeak * (t - self.t_acc)
        else:
            td = t - (self.t_acc + self.t_flat)
            d = (0.5 * a * self.t_acc ** 2 + self.vpeak * self.t_flat +
                 self.vpeak * td - 0.5 * a * td ** 2)
        return self.start + self.direction * d


def make_synced_profiles(targets, vmax, amax):
    """
    targets: dict axis_key -> (start, end)
    Returns (profiles, duration) where every axis's profile has been
    slowed down (lower peak velocity, same accel) so that all axes arrive
    at their target at the same time as the axis that needs the longest.
    This keeps a straight, predictable joint-space motion instead of one
    axis snapping ahead of the other.
    """
    raw_profiles = {ax: TrapezoidalProfile(s, e, vmax, amax) for ax, (s, e) in targets.items()}
    duration = max((p.duration for p in raw_profiles.values()), default=0.0)
    if duration <= 0:
        return raw_profiles, 0.0

    synced = {}
    for ax, (s, e) in targets.items():
        if abs(e - s) < 1e-9:
            synced[ax] = raw_profiles[ax]
            continue
        lo, hi = 1e-6, vmax
        v = vmax
        # binary-search the peak velocity for this axis so its profile
        # duration matches the slowest axis
        for _ in range(30):
            p = TrapezoidalProfile(s, e, v, amax)
            if abs(p.duration - duration) < 1e-4:
                break
            if p.duration < duration:
                ...
                lo = v
                v = (v + hi) / 2.0
        synced[ax] = TrapezoidalProfile(s, e, v, amax)
...
class FiveBarDashboard:
    def __init__(self):
        ...
        self.trajectory_points = []      # actual end-effector path traced during a move
        self.planned_path_points = []    # preview / target path for path planning
        self.waypoints = []              # list of (x, y) mm for path planning

        # last-known FK result, used to redraw the viz without hardware access
        self.last_P1 = None
        self.last_P2 = None
        self.last_E = None
    ...
    def build_ui(self):
        ...
        with ui.row().classes("w-full items-center justify-between p-2"):
            ...
            with ui.row().classes("items-center"):
                ui.button("Stop Motion", on_click=self.stop_motion,
                           color="orange").classes("font-bold")
    ...
    async def stop_motion(self):
        # Softer than EMERGENCY STOP: halts any commanded motion by holding
        # the axes at their current encoder position, but leaves them in
        # CLOSED_LOOP_CONTROL (motors stay engaged/holding torque).
        ...
        def _do():
            pos0 = self.odrv0.axis0.encoder.pos_estimate
            pos1 = self.odrv0.axis1.encoder.pos_estimate
            self.odrv0.axis0.controller.input_pos = pos0
            self.odrv0.axis1.controller.input_pos = pos1

        try:
            await run.io_bound(_do)
            self.log("Stop Motion: axes held at current position.")
        except Exception as e:
            self.log("Stop Motion failed: {}".format(e))
