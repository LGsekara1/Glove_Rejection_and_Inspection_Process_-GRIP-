...
DEFAULT_PROJECT_ROOT = Path(r"D:\GRIP_Yolo_pose")
DEFAULT_OUTPUT_NAME = "YOLO_pose_MVP_dataset"
...
Draw one bounding box around each visible glove.

Then place the 7 keypoints:

- wrist_center: center of cuff/wrist opening side
- palm_center: approximate center of palm area
- thumb_tip: visible thumb tip. If hidden, mark as occluded or leave as missing
- index_tip: tip of index finger
- middle_tip: tip of middle finger
- ring_tip: tip of ring finger
- pinky_tip: tip of pinky finger
...
Do not use single horizontal flip for this dataset during training.
...
The system should learn glove anatomy and keypoint geometry. The final MVP decision should be based on finger-order/keypoint geometry, not just class name.

## Output folders

- frames_to_label: extracted frames ready to annotate
...
class PoseAnnotator:
    ...
    def build_ui(self):
        ...
        self.side = ttk.Frame(self.main, width=390)
        ...
        for i, name in enumerate(KEYPOINT_NAMES):
            ...
            ttk.Radiobutton(
                self.side,
                text=text,
                variable=self.selected_keypoint,
                value=i
        ...
        help_text = (
            ...
            "class xc yc w h x1 y1 v1 ... x7 y7 v7"
        )
    ...
    def bind_events(self):
        ...
        for i in range(7):
            self.root.bind(str(i + 1), lambda e, idx=i: self.selected_keypoint.set(idx))
    ...
    def draw_annotation(self, ann, selected=False):
        ...
        label = "glove"
        self.canvas.create_rectangle(x1, y1 - 22, x1 + 95, y1, fill=outline, outline=outline)
    ...
    def update_object_list(self):
        ...
        for i, ann in enumerate(self.annotations):
            ...
            text = f"{i + 1}. glove | keypoints {visible_count}/7"
    ...
    def save_preview(self, img_path):
        ...
        for ann in self.annotations:
            ...
            draw.rectangle([x1, max(0, y1 - 24), x1 + 100, y1], fill=(0, 255, 0))
            draw.text((x1 + 4, max(0, y1 - 22)), "glove", fill=(0, 0, 0))
...
class MainApp:
    ...
    def build_ui(self):
        ...
        ttk.Button(buttons, text="1. Extract Frames", command=self.extract_frames).pack(fill=tk.X, pady=5)
        ttk.Button(buttons, text="2. Open YOLO-Pose Annotator", command=self.open_annotator).pack(fill=tk.X, pady=5)
        ttk.Button(buttons, text="3. Create 70/20/10 YOLO-Pose Split", command=self.create_split).pack(fill=tk.X, pady=5)
        ttk.Button(buttons, text="Open Output Folder", command=self.open_output).pack(fill=tk.X, pady=5)

        info = (
            "Workflow:\n"
            "1. Extract frames from your raw videos.\n"
            "2. Annotate each glove with bbox + 7 keypoints.\n"
            "3. Create train/val/test split for YOLO-pose.\n\n"
            "Output is saved inside your New_model folder."
        )
